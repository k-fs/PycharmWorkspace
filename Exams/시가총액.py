...
class CpMarketEye:
    ...
    def Request(self, codes, dataInfo):
        ...
        for i in range(cnt):
            ...
            junga = self.objRq.GetDataValue(3, i)  # 전일가격
...
#            print(code, maketAmt)
            ...
            # key(종목코드) = tuple(상장주식수, 현재가, 시가총액, 전일가격)
            dataInfo[code] = (listedStock, cur, maketAmt, junga)
...
#        print('전종목 시가총액 순 조회 (%d 종목)' % (len(data2)))
            for item in data2:
                ...
                #Sample select query
                curs.execute("update oms9jj set eSangJangSu = %s, eHyunJaeGa = %s , eJunilGa = %s where eToDate = '%s' and eStockCode = '%s'" % (listed, cur, junga, today, code))
#                curs.execute("update oms9jj set eHyunJaeGa = %s where eToDate = '%s' and eStockCode = '%s'" % (cur, today, code))
